...
You can create a link to automatically add your AltSource by using this template: ```sidestore://source?url=https://yoursourcehere.com/apps.json```. For more info, see (URL Schema)[advanced/url-schema]. It is often helpful to create the link to your AltSource in this format, then put it into a link shortener such as ```tinyurl.com```. However, a user tries to use a sidestore:// without SideStore already installed, it will crash, so some documentation may be helpful.
