...
### Lara (3-app limit)
:::note
 the Lara "bypass 3 app limit" exploit uses the darksword exploit which **only works* on 17.0 - iOS 18.7.1 & iOS 26.0.x, excluding M5 and A19. also has worked on 16.7.2.
If you are not on these versions dont ask for support.
:::
To use this make sure you are on the latest nightly version of Sidestore and install [lara](https://github.com/rooootdev/lara/releases/) (works on livecontainer)
1.Click run exploit in lara app
2.(only if it is your first time running the exploit) Click fetch kernalcache
3.Click initialize system
4.Click the lara logo at the bottem middle of the screen
5.Under the apps section click 3 app bypass
6.Click Bypass
:::Warning
DO NOT close out lara from the app switcher yet you can after step the end
:::
7.Go to sidestore and install ipa
8.Profit
:::note
This does not change the ten app id limit
:::
You must do this for every new app you sidelode