---
title: Burner Apple ID
description: A guide on burner Apple IDs.
---

The recommended way to use SideStore is with a burner Apple ID account. This avoids temporarily losing an important account due to logins falsely flagged as malicious if using an old anisette server method that's not anisette v3 or above.
Trying to use an old method of anisette will notify in app if it is not a safe version of anisette (Able to lock accounts which just forces password reset which is able to be prevented on v3 and above anisette versions).
...
4. Add a first and last name, and enter a date older than 18 years.
