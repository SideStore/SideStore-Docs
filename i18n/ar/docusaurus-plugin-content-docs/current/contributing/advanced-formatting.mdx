...
#  إرشادات تنسيق الكود المتقدمة

سنتناول حالات التنسيق المتقدمة، وأنماط البنية البرمجية، وميزات Swift وObjective-C المعقدة الخاصة بمشروع SideStore.

## أنماط لغة Swift المتقدمة

### البرمجة الموجهة بالبروتوكولات

#### تعريفات البروتوكول
- حافظ على تركيز البروتوكولات وتماسكها
- استخدم أنواع شائعة للبروتوكولات المرتبطة
- وفّر تطبيقات افتراضية في الامتدادات عند الحاجة
...
#### تركيب البروتوكولات
- استخدم بروتوكولات مركبة عند التعامل مع متطلبات معقدة
- اجعل البروتوكولات الفردية موجزة ومركزة
...
### الأنماط الشائعة وقيود الأنواع

#### تعريفات الأنواع الشائعة
- استخدم أسماء قيود ذات دلالة
- فضّل استخدام قيود البروتوكولات على قيود الفئات
- استخدم عبارات `where` للقيود المعقدة
...
#### قيود الأنواع الشائعة المتقدمة
- استخدم المطابقة الشرطية بالشكل المناسب
- استخدم الأنواع الوهمية عند الحاجة
...
#### تصميم الدوال غير المتزامنة (Async)
- استخدم async/await بشكل باستمرار
- نظِّم العمليات المتزامنة بوضوح
- تعامل مع عملية الإلغاء بشكل مناسب
...
actor AppInstallationManager {
    ...
    private func performInstallation(_ app: App) async throws {
        // تحقق من وجود إلغاء عند نقطة مهمة
...
func installApp(_ app: App, completion: @escaping (Error?) -> Void) {
    DispatchQueue.global().async {
        // الجمع بين أسلوب معالجات الإكمال التقليدي وكود async البرمجي الجديد
...
#### التزامن المُهيكل
- استخدم مجموعات المهام للعمليات المتزامنة المترابطة
- يفضل التزامن المنظم على المهام غير المنظمة
...
func installMultipleApps(_ apps: [App]) async throws {
    try await withThrowingTaskGroup(of: Void.self) { group in
        ...
        // انتظر حتى تكتمل جميع عمليات التثبيت
        try await group.waitForAll()
    }
}

// للحصول على نتائج مستقلة
...
### بناء النتائج ولغات النطاق المخصص (DSLs)

#### مُنشِئات النتائج المخصصة
- أنشئ مُنشئات نتائج مُركّزة ومُخصصة لغرض واحد
- وفِّر صيغةً واضحةً للعمليات الخاصة بالمجال
...
## أنماط Objective-C المتقدمة

### تنظيم الفئات
- استخدم الفئات لتنظيم الوظائف المترابطة
- اجعل أسماء الفئات وصفية ومحددة
...
- استخدم الأنماط المناسبة لعلاقات التفويض
- تعامل مع مخططات الكائنات المعقدة بشكل صحيح
...
### أنماط استخدام الكتل البرمجية
- استخدم أنماط الكتل المناسبة للعمليات غير المتزامنة.
- تعامل مع إدارة الذاكرة في الكتل بشكل صحيح
...
### تطبيق نمط MVVM
- افصل بوضوح بين الاهتمامات المتعلقة بالنموذج، والعرض، ونموذج العرض
- استخدم أنماط ربط البيانات المناسبة
...
### حقن التبعيات
- استخدم حقن التبعية لضمان قابلية الاختبار والمرونة
- استخدم حاوية حقن التبعية (DI container) للتطبيقات المعقدة
...
### بنية معالجة الأخطاء
- ضع استراتيجيات شاملة لمعالجة الأخطاء
- استخدم الأخطاء المكتوبة لتحسين معالجة الأخطاء
...
### تحسين الذاكرة
- استخدم التحميل الكسول للموارد المكلفة
- طبق استراتيجيات التخزين المؤقت المناسبة
...
class AppImageCache {
    ...
    func image(for app: App) async -> UIImage? {
        ...
        // قم بالتنزيل إذا لم يتم حفظه في ذاكرة التخزين المؤقت
...
### أفضل الممارسات لتنفيذ المتعدد (Threading)
- استخدم أولويات الطابور المناسبة
- قلل من context switching
...
### الاختبار القائم على البروتوكول
- استخدم البروتوكولات لحقن التبعيات في الاختبارات
- أنشئ بدائل اختبار مُركّزة
...
### اشرح استعمال الـ API المعقد
- توثيق السلوكيات المعقدة والحالات الاستثنائية
- قدم أمثلة لستخدام APIs معقدة
 ...
 * ```
 *
 * ## سلامة التنفيذ المتعدد
 * يجب ان تكون الفئة آمنة للاستخدام المتزامن (thread-safe) ويمكن استدعاؤها من أي طابور
 * يتم استدعاء المعالِجات في طابور المهام الرئيسي ما لم يُنصَّ على خلاف ذلك.
 *
 * ## معالجة الأخطاء
 * تُصنَّف حالات فشل التثبيت إلى أخطاء قابلة للمعالجة وأخرى غير قابلة للمعالجة.
 * بالنسبة للأخطاء القابلة للمعالجة (مثل انتهاء مهلة الشبكة أو المشكلات المؤقتة في الجهاز)،
 * ستتم إعادة المحاولة تلقائياً حتى بلوغ الحد المحدد. أما الأخطاء غير القابلة للمعالجة
 * (مثل التوقيعات غير الصالحة أو الأجهزة غير المتوافقة)، فستؤدي إلى الفشل فوراً.
 */
@MainActor
...
تذكر: ينبغي استخدام هذه الأنماط المتقدمة بحكمة. أعطِ الأولوية دائمًا لوضوح الكود وسهولة صيانته بدلًا من اللجوء إلى تنفيذات معقدة أو ذكية أكثر من اللازم. وعند الشك، اختر النهج الأبسط الذي لا يزال يلبّي المتطلبات.